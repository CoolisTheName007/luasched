...
sched.is_running=function()
	return sched and sched.proc.tasks.running
end

sched.rpl_os={}
local rpl=sched.rpl_os
rpl.out_os={
pullEvent		=os.pullEvent,
pullEventRaw	=os.pullEventRaw,
pullEvent		=os.pullEvent,
}
--more efficient versions of read, sleep, (turtle?) TBD
rpl.in_os={
pullEventRaw= function (event)
	log.trace ('fd', 'DEBUG', "pullEventRaw %s", event)
	return select(2,sched.wait(sched,event))
end,
pullEvent = function (event)
	log.trace ('fd', 'DEBUG', "pullEvent %s", event)
	event,p1,p1,p3,p4,p5=select(2,sched.wait(sched,event))
...
sleep = function (n)
	log.trace ('fd', 'DEBUG', "os sleep %d", n)
	sched.wait(n)
end,
}

rpl.all_os={}
for i,v in pairs(rpl.out_os) do
	rpl.all_os[i]=function(...)
		if sched and sched.proc.tasks.running then
			return rpl.in_os[i](...)
		else
			return v(...)
		end
